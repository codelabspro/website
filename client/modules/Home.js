...
// react-famous components (from my reacfamo fork).
        ...
        for(var x=0; x<col; x++){
            for(var y=0; y<row; y++){
                ...
                node = this.rootNode.addChild()
                    ...
                    .setOpacity(z);
                sphere = new Sphere({mass: 50, radius: 100, position:new Vec3(left,top)});
...
class Home extends Node {
    render() {
        return (
            <Node _famousParent={this} ref="wrapper">
            </Node>
        )
    }

    // TODO handle mount and unmount for garabage collection.
    componentDidMount() {
        super.componentDidMount()

        let rootNode = this.refs.wrapper._famousNode

        // XXX We can convert _Home and ExplodingTriangles into react-famous
        // components if we wanted to too.
        var bootstrap = new _Home(rootNode);
        var web = new ExplodingTriangles(rootNode, 128);
    }
}
